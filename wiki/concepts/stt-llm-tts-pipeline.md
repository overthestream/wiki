...
sources: ["raw/articles/LiveKit Documentation.md", "raw/articles/Realtime conversations  OpenAI API.md"]
created: 2026-04-21
updated: 2026-04-21
---
...
### 왜 파이프라인인가 — Speech-to-Speech 모델과의 비교

[[entities/openai-realtime-api|OpenAI Realtime API]]·[[entities/amazon-nova-sonic|Nova Sonic]] 같은 "음성→음성 직접" 모델이 등장했지만, cascade 파이프라인은 여전히 다음 이유로 주류다:
...
반대로 s2s 모델은 **톤/억양 등 오디오의 프레이징 정보를 그대로 소비**하며, STT→LLM→TTS 체인에서 손실되던 prosody 정보를 활용할 수 있다는 강점이 있다. 또한 첫 토큰까지의 누적 지연이 적다.

cascade 단점은 **누적 지연**이다. STT 끝 → LLM 첫 토큰 → TTS 첫 오디오까지의 chain 지연이 사용자 체감 응답성을 결정한다. 따라서 각 단계를 **스트리밍으로 맞물리게 하는 것**이 구현의 핵심.
